...
/// Citadel rule / connection / profile persistence.
/// Schema evolves only via the `schema_migrations` table (additive migrations).
public final class RuleStore: @unchecked Sendable {
    ...
    private func migrate() throws {
        ...
        let applied = queue.sync { () -> Int in
            var stmt: OpaquePointer?
            defer { if stmt != nil { sqlite3_finalize(stmt) } }
            guard sqlite3_prepare_v2(db, "SELECT COALESCE(MAX(version), 0) FROM schema_migrations;", -1, &stmt, nil) == SQLITE_OK else {
                return 0
            }
    ...
    private func recordMigration(_ version: Int) throws {
        try execute("INSERT OR IGNORE INTO schema_migrations(version, applied_at) VALUES (?, ?);") { stmt in
            sqlite3_bind_int(stmt, 1, Int32(version))
            sqlite3_bind_double(stmt, 2, Date().timeIntervalSince1970)
        }
    }

    private func applyV1BaseSchema() throws {
        try exec("""
    ...
    private func seedProfiles() throws {
        ...
        for profile in defaults { try insertProfileIfMissing(profile) }
    }
    ...
    private func seedBlocklists() throws {
        ...
        for list in defaults { try insertBlocklistIfMissing(list) }
    }
    ...
    // MARK: - Profiles / blocklists / settings / connections
