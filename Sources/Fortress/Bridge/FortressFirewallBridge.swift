...
/// Fortress firewall façade — delegates to `AppState` so Network Extension
/// SharedRuleBridge stays in sync. No second XPC client (alerts stay on AppState).
@MainActor
final class FortressFirewallBridge: ObservableObject {
    @Published private(set) var connected = false
    @Published private(set) var mode: AppMode = .alert
    @Published var pendingAlerts: [FortressAlert] = []
    @Published private(set) var rules: [Rule] = []

    weak var viewModel: FortressViewModel?
    ...
    func resolveAlert(_ alert: FortressAlert, allow: Bool, remember: Bool) {
        // Prefer AppState floating alert path; this is for any residual Fortress queue.
    ...
    private func makeRule(from stream: NetworkStream, action: RuleAction, remember: Bool) -> Rule {
        Rule(
            ...
            requiresSignature: stream.process.signingStatus == .signedValid,
            ...
            priority: 100,
            notes: remember ? "Created from Fortress" : "Temporary from Fortress",
            temporary: !remember,
