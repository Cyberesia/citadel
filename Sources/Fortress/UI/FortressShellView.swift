...
/// Root Fortress experience — independent of classic NetworkMonitorView.
struct FortressShellView: View {
    @EnvironmentObject var vm: FortressViewModel
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            FortressToolbar(vm: vm)
            Divider().opacity(0.25)

            HStack(spacing: 0) {
                FortressNavTree(vm: vm)
                    .frame(width: 260)
    ...
    private var statusBanner: some View {
        if vm.isDemoMode {
            banner(
                icon: "sparkles",
                title: L10n.fortressDemo,
                subtitle: L10n.fortressDemoSubtitle,
    ...
    private func alertBanner(_ alert: FortressAlert) -> some View {
        HStack(spacing: 12) {
