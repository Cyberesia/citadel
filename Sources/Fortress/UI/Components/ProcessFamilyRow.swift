...
struct ProcessFamilyRow: View {
    ...
    private var rateLabel: String {
        if node.rateTotal > 0 { return FortressFormat.bytesPerSec(node.rateTotal) }
        if node.bytesTotal > 0 { return FortressFormat.bytes(node.bytesTotal) }
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(stream.rateTotal > 0
                 ? FortressFormat.bytesPerSec(stream.rateTotal)
                 : FortressFormat.bytes(stream.bytesTotal))
                .font(.ps(10, weight: .semibold))
                .foregroundStyle(PrismTheme.textSecondary)
        }
        .padding(.horizontal, 10)
