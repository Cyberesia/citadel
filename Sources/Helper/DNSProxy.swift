...
final class DNSProxy: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.citadel.firewall.dns", qos: .userInitiated)
    private let upstreamQueue = DispatchQueue(label: "com.citadel.firewall.dns.up")
    ...
    private func process(payload: Data, isTCP: Bool, reply: @escaping (Data?) -> Void) {
        ...
        let action = evaluator.decision(for: connStub, rules: rules, defaultMode: mode)
