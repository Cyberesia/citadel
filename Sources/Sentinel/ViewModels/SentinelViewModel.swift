...
final class SentinelViewModel: ObservableObject {
    ...
    @Published var arcs: [SentinelFlowArc] = []
    @Published var options = MonitorViewOptions.default
    @Published var mapProjection: SentinelMapProjection = .globe
    @Published var mapOrigin = SentinelGeoPoint.defaultOrigin
    @Published var currentIn: Int64 = 0
    @Published var currentOut: Int64 = 0
    @Published var machineSparkline: [Int64] = []
    @Published var topDomains: [SentinelRollup] = []
    @Published var topCountries: [SentinelRollup] = []
    @Published var topFamilies: [SentinelRollup] = []
    @Published var topDestinations: [SentinelRollup] = []
    ...
    let bridge = SentinelFirewallBridge()
    ...
    var pendingAlerts: [SentinelAlert] { bridge.pendingAlerts }
    ...
    func bind(appState: AppState) {
        ...
        appState.adoptSentinelTelemetry()
        menubarSink = appState
    }

    private weak var menubarSink: AppState?

    /// Mirror Sentinel rates + families into AppState for menubar / popover.
        ...
        originTask = Task { [weak self] in
            if let loc = await GeoResolver.shared.lookupSelf(),
               let lat = loc.latitude, let lon = loc.longitude {
                await MainActor.run {
                    self?.mapOrigin = SentinelGeoPoint(latitude: lat, longitude: lon)
                }
            }
        }
        rebuildPresentation()
    ...
    func loadDemo() {
        stopEngine()
        isDemoMode = true
        SentinelDemoData.load(into: self)
        rebuildPresentation()
    }

    /// Leave demo and resume live Sentinel telemetry.
    func exitDemo() {
    ...
    func copyDetails(for stream: NetworkStream) {
        ...
        Rate: ↓\(SentinelFormat.bytesPerSec(stream.rateIn)) ↑\(SentinelFormat.bytesPerSec(stream.rateOut))
    ...
    private func rebuildRollups() {
        ...
        topCountries = countries.map {
            SentinelRollup(id: $0.key, label: $0.value.label, rateTotal: $0.value.rate, bytesTotal: $0.value.bytes, connectionCount: $0.value.count)
    ...
    private func rebuildArcs() {
        ...
        var built = SentinelFlowArcBuilder.build(
            from: source,
            origin: mapOrigin,
        ...
        if built.isEmpty {
            ...
            case .stream(let id):
                if let familyID = streams.first(where: { $0.id == id })?.process.familyID {
                    ...
                    built = SentinelFlowArcBuilder.build(
                        from: source,
                        origin: mapOrigin,
                        highlightStreamID: selectedStreamID
                    )
