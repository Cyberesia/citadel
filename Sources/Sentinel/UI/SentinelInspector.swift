...
struct SentinelInspector: View {
    ...
    private func streamDetail(_ stream: NetworkStream) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            detailRow(L10n.rateDown, SentinelFormat.bytesPerSec(stream.rateIn))
            detailRow(L10n.rateUp, SentinelFormat.bytesPerSec(stream.rateOut))
    ...
    private var overview: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 8) {
                ratePill(SentinelFormat.bytesPerSec(vm.currentIn), L10n.trafficDown, PrismTheme.trafficDown.opacity(0.35))
                ratePill(SentinelFormat.bytesPerSec(vm.currentOut), L10n.trafficUp, PrismTheme.trafficUp.opacity(0.35))
            }
    ...
    private func rollupSection(
        _ title: String,
        items: [SentinelRollup],
        onSelect: @escaping (SentinelRollup) -> Void
    ) -> some View {
                    ...
                    Button {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text(item.rateTotal > 0
                                 ? SentinelFormat.bytesPerSec(item.rateTotal)
                                 : SentinelFormat.bytes(item.bytesTotal))
                                .font(.ps(10, weight: .semibold))
                                .foregroundStyle(PrismTheme.textSecondary)
                                .monospacedDigit()
                        }
