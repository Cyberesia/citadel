...
/// Sentinel-native rules studio — create, suggest from activity, fine-tune.
struct SentinelRulesView: View {
    ...
    var body: some View {
        ...
        .onAppear { sentinel.bridge.refreshRules() }
        .sheet(isPresented: $showComposer) {
            RuleComposerSheet(
                draft: $composerDraft,
                helperOnline: sentinel.helperConnected,
                onSave: { draft in
                    let rule = draft.makeRule()
                    sentinel.bridge.addRule(rule)
                    selected = rule
                    showComposer = false
                },
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 6) {
                PrismActionChip(title: L10n.allow, systemImage: "checkmark", kind: .allow) {
                    sentinel.bridge.addRule(s.asRule(action: .allow))
                }
                PrismActionChip(title: L10n.deny, systemImage: "xmark", kind: .deny) {
                    sentinel.bridge.addRule(s.asRule(action: .deny))
                }
            }
        }
        .padding(10)
            ...
            if let rule = selected {
                ...
                VStack(spacing: 8) {
                    ...
                    PrismActionChip(title: L10n.remove, systemImage: "trash", kind: .deny) {
                        sentinel.bridge.removeRule(id: rule.id)
    ...
    private var filteredRules: [Rule] {
        var list = sentinel.bridge.rules
        switch filter {
        case .all: break
    ...
    private func count(for f: RuleFilter) -> Int {
        let list = sentinel.bridge.rules
        switch f {
    ...
    private var suggestions: [RuleSuggestion] {
        ...
        let existingHosts = Set(sentinel.bridge.rules.compactMap(\.remoteHost))
        let existingFamilies = Set(sentinel.bridge.rules.compactMap(\.processName))

        for family in sentinel.topFamilies.prefix(4) {
        ...
        for dest in sentinel.topDestinations.prefix(4) {
            guard let stream = sentinel.stream(for: dest.id) else { continue }
...
struct RuleSuggestion: Identifiable {
    ...
    func asRule(action: RuleAction) -> Rule {
        Rule(
            ...
            scope: scope,
            priority: 100,
            notes: L10n.suggestedFromSentinel
        )
    }
}
...
struct RuleComposerDraft {
    ...
    func makeRule() -> Rule {
        Rule(
            ...
            scope: scope,
            priority: 100,
            notes: notes.isEmpty ? L10n.createdInSentinelRules : notes,
            temporary: temporary,
