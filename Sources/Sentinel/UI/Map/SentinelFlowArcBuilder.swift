...
enum SentinelMapProjection: String, CaseIterable, Identifiable, Sendable {
    case flat
    case globe
...
enum SentinelFlowArcBuilder {
    ...
    private static func destination(for stream: NetworkStream) -> SentinelGeoPoint? {
        if let geo = stream.geo, let lat = geo.latitude, let lon = geo.longitude {
            return SentinelGeoPoint(latitude: lat, longitude: lon)
        }
        if let code = stream.geo?.countryCode,
           let point = CountryCentroids.point(for: code) {
            return SentinelGeoPoint(latitude: point.latitude, longitude: point.longitude)
        }
        return nil
    }
}
